...
(function (A05_EinkaufslisteClient) {
    ...
    A05_EinkaufslisteClient.startGenerateContent = startGenerateContent;
    ...
    function deleteItem() {
        ...
        destroyContent(A05_EinkaufslisteClient.shoppingList);
        A05_EinkaufslisteClient.shoppingList.Liste.splice(activeID, 1);
        generateContent(A05_EinkaufslisteClient.shoppingList);
    }
    function updateItem() {
        ...
        A05_EinkaufslisteClient.shoppingList.Liste[getButtonID()].quantity = parseInt(newAmountString);
        A05_EinkaufslisteClient.shoppingList.Liste[getButtonID()].comment = newCommentString;
        destroyContent(A05_EinkaufslisteClient.shoppingList);
        generateContent(A05_EinkaufslisteClient.shoppingList);
    }
    function changeItemStatus() {
        ...
        let ActiveID = parseInt(getButtonID());
        if (A05_EinkaufslisteClient.shoppingList.Liste[ActiveID].inCart == false && A05_EinkaufslisteClient.shoppingList.Liste[ActiveID].bought == false) {
            ActiveButton.classList.add("fa-cart-shopping");
            ActiveButton.classList.remove("fa-circle");
            A05_EinkaufslisteClient.shoppingList.Liste[ActiveID].inCart = true;
        }
        else if (A05_EinkaufslisteClient.shoppingList.Liste[ActiveID].inCart == true && A05_EinkaufslisteClient.shoppingList.Liste[ActiveID].bought == false) {
            ActiveButton.classList.add("fa-check");
            ActiveButton.classList.remove("fa-cart-shopping");
            A05_EinkaufslisteClient.shoppingList.Liste[ActiveID].bought = true;
        }
        else {
            ActiveButton.classList.add("fa-circle");
            ActiveButton.classList.remove("fa-check");
            A05_EinkaufslisteClient.shoppingList.Liste[ActiveID].inCart = false;
            A05_EinkaufslisteClient.shoppingList.Liste[ActiveID].bought = false;
        }
    }
