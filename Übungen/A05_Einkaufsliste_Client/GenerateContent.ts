...
{
    export async function startGenerateContent(_url: RequestInfo): void
    {
        let response: Response = await fetch(_url);
        console.log("Response", response);
        let shoppingJSON: Promise<JSON> = response.json();
        let shoppingList: ShoppingList = JSON.parse(shoppingJSON);
        generateContent(shoppingList);
    }
    
    export function generateContent(_shoppingList: ShoppingList): void
    {
        let listSpace: HTMLElement = document.getElementById("list");
